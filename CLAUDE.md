...
- **Proto package**: `analytics_iface.v1` with **three services** split by domain:
  ...
  - `InventoryAnalyticsService` — `GetTurnover`, `GetDeadStock`, `GetDaysOfStockRemaining`, `GetExpiryRiskForecast` (30d/90d/180d). **All four are scoped to the caller's active warehouse** (`resolveWarehouse` → `AND sm.warehouse_id = ?` on every `stock_movements` reference); the owner switches warehouses via the TopBar picker. No "all warehouses" toggle.
...
- **CSV export = client-side.** [lib/csv.ts](frontend/src/lib/csv.ts) `downloadCsv(filename, rows, columns)` serializes rows, prepends a UTF-8 BOM (so Excel doesn't mangle non-ASCII), and triggers a download. No server-side export RPC. Analytics serializes the already-fetched page rows. **The server-paginated list pages (Order history, Restock, Mutasi, Transfer, Medicine) use the shared `<ExportButton>`** ([components/ExportButton.tsx](frontend/src/components/ExportButton.tsx)): an async handler fetches **all** rows matching the current filters at `limit: ALL_LIMIT` (1000) via a `fetch<Domain>ForExport` adapter in the domain's queries file, resolves referenced names where the rows carry only IDs (`resolveSupplierMap` / `resolveBatchMap` in [queries/refs.ts](frontend/src/queries/refs.ts) — dedupe + chunk to the 500-id resolve cap), and serializes (money as raw whole-rupiah numbers, dates/enums localized). Beyond 1000 rows needs a server-side export RPC.
