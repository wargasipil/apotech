	...
	base := func() *gorm.DB {
		...
		if query := strings.TrimSpace(req.Msg.Query); query != "" {
			...
			sub := b.db.Table("batches AS b2").
				...
				Where(fmt.Sprintf("b2.batch_number %[1]s ? OR m.name %[1]s ? OR m.sku %[1]s ?", likeKeyword(b.db)), pattern, pattern, pattern)
...
func (b *Batches) SearchBatches(
	...
	if query != "" {
		pattern := "%" + query + "%"
		q = q.Where(fmt.Sprintf("b.batch_number %[1]s ? OR m.name %[1]s ? OR m.sku %[1]s ?", likeKeyword(b.db)), pattern, pattern, pattern)
	}
