...
// dailyOrderMetric is build-tag split — analytics_postgres.go uses DATE_TRUNC,
// analytics_sqlite.go uses strftime. Both return the same map shape keyed by
// dayBucketKey output.
...
// productPageIDs builds the universe of candidate medicine ids (union of any
// medicine with an order in range OR any medicine with stock in the active
	...
	combined := `
		WITH order_agg AS (
		  SELECT si.medicine_id,
		         ...
		         COALESCE(` + sqldialect.UnixEpoch("MAX(s.completed_at)") + `, 0) AS last_order_unix
		  FROM sale_items si
		...
		ongoing_agg AS (
		  SELECT poi.medicine_id, COALESCE(SUM(poi.ordered_qty - poi.received_qty), 0) AS ongoing
		  FROM purchase_order_items poi
		  JOIN purchase_orders po ON po.id = poi.purchase_order_id
		  WHERE po.status NOT IN ('VOIDED', 'CLOSED', 'RECEIVED')
		...
		restock_agg AS (
		  SELECT b.medicine_id,
		         ` + sqldialect.UnixEpoch("MAX(b.received_at)") + ` AS last_restock_unix
		  FROM batches b
		...
		expiring_agg AS (
		  ...
		  WHERE b.expiry_date >= CURRENT_DATE
		    AND b.expiry_date < `+sqldialect.DatePlusDays(30)+`
		  GROUP BY b.medicine_id
		),
		combined AS (
		  SELECT m.id, m.name,
